...
import static apiHelper.services.deleteBooking.deleteBookingRequest.deleteAlreadyDeletedBookingValidation;
...
public class deletePostEndPoint extends BaseTest {
    ...
    @Test
    @Parameters({"env"})
    public static void validateDeleteBookingEndPointForAnAlreadyDeletedBookingId(String env, Method method) {
        ExtentTest extentTest = startTest(method.getName(), "Validate delete booking end point");
        extentTest.log(Status.INFO, "Execution starts for " + method.getName());
        String token = getTokenValue(env, CREATE_TOKEN, "createAuth.json");
        String bookingId = "3";
        deleteAlreadyDeletedBookingValidation(env, DELETE_BOOKING, bookingId, token, extentTest);
        extentTest.log(Status.INFO, "Execution completed for " + method.getName());
    }
}
